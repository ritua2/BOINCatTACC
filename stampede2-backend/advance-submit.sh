...
if [ $server == "boinc" ]; then
  read -p "Do you want to proceed to run the job on BOINC server?";
  if [ $REPLY != "y" -o REPLY != "Y" ]; then
    read -p "Do you want to run the job on TACC resourses only"
    if [ $REPLY == "y" -o REPLY == "Y" ]; then
  ...
  if [ $slurmpresent == "n" ] ; then
    echo "Based on questionaire the job qualifies to run on TACC System but there is no Slurm job available"
    echo "Exiting..."
    exit 0
  fi
fi
case $server in
  boinc)
    ...
    read filetosubmit
    echo "$filetosubmit"
    echo -n "Enter the your user email id for job status to be sent: "
    read useremail
    echo -n "Enter the boinc server ip address : "
    read SERVER_IP

    curl -F file=@$filetosubmit http://$SERVER_IP:5075/boincserver/v2/submit_known/email=$useremail
