...
function move_midas_input_file($folderName, $setupFile){
	$target_dir = "/etc/".$folderName."/";
...
function get_json(){
	...
	if(isset($_FILES["setup_file"]) || isset($_FILES["midas_input_file"])){
		create_folder();
		if(isset($_FILES["setup_file"]))
			move_midas_input_file($theFolderName, $_FILES["setup_file"]["tmp_name"]);
		if(isset($_FILES["midas_input_file"]))
	...
	$tempData = array(
					  ...
					  'programming_language'=>get_prog_lang_list(), 
...
function submit_midas_job(){
	...
	curl_setopt($ch, CURLOPT_URL, "http://0.0.0.0:6075/boincserver/v2/api/process_midas_jobs");
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_POSTFIELDS, get_json());
