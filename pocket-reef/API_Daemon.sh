...
if [ $1 == "-up" ]; then 
   ...
   nohup /reef/new_user.py & \
         > /dev/null 2>&1 & echo $! > ./nnuu_api.txt
   nohup /reef/reef_regular.py & \
        > /dev/null 2>&1 & echo $! > ./rerg_api.txt
   nohup /reef/reef_results.py & \
        > /dev/null 2>&1 & echo $! > ./rers_api.txt
...
if [ $1 == "-down" ]; then 
   ...
   kill -9 $(($(cat ./nnuu_api.txt) - 1))
   kill -9 $(($(cat ./rerg_api.txt) - 1))
   kill -9 $(($(cat ./rers_api.txt) - 1))
