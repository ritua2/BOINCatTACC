...
echo '<pre style="word-break:normal;white-space:normal;margin-left:20%;margin-right: 20%;"><style="text-align:left;"><p style="font-size:14px;font-weight: normal;"><span style="font-weight:bold;">July 30th, 2018:</span> TACC-2-BOINC project goes live today. Volunteers are now invited to join the project and donate the compute cycles. Individual volunteers can create their accounts on the TACC-2-BOINC website and download the BOINC client for donating compute-cycles on their devices or VMs that they own in the cloud. Institutional volunteers, can email Ritu Arora at rauta@tacc.utexas.edu, to learn more about donating compute cycles. </p>';

echo '<p style="font-size:14px;font-weight: normal;">TACC-2-BOINC can be used by the TACC researchers to run applications written in C, C++, Fortran, Python, Bash. The software infrastructure behind TACC-2-BOINC relies on the availability of the applications as Docker images. TACC researchers have the option of using the pre-built Docker images of the following applications that are maintained by us: Autodock-Vina, Bedtools, BLAST, Bowtie, GROMACS, HTSeq, LAMMPS, NAMD, and OpenSees. Researchers can also choose to run any other publicly available image in Docker Hub. If the researchers are not familiar with containerization, they can provide their source code with the instructions to build the executable, and our software framework can create the Docker image for them. </p>';

echo '<p style="font-size:14px;font-weight: normal;">Happy computing with TACC-2-BOINC!</p></style></pre></br>';
